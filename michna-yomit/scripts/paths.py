...
Elles vont dans le dossier de travail, dans cet ordre de priorite :
  1. $MICHNA_HOME
  2. un fichier .michna-yomit dans le repertoire courant (vide = travailler
...
SEED_FILES = ("tractates.json", "program.json", "dafyomi_slugs.json",
              "calendar.json")
...
def _resolve_home() -> str:
    env = os.environ.get("MICHNA_HOME")
    if env:
    ...
    if os.path.exists(os.path.join(cwd, "data", "program.json")):
        return cwd
    return os.path.expanduser("~/Documents/Michna Yomit")
